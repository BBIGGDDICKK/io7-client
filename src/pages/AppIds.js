...
const AppIds = () => {
  ...
  const rows = appIds.map((app) => ({
    ...
    appDesc: app.appDesc,
    navigate: (
      <IconButton onClick={() => setChosenApp(app)}>
        <AppRegistrationIcon />
      </IconButton>
    ),
  }));
        ...
        {chosenApp ? (
          ...
          <>
            ...
            <section className="mt-8">
              <h3 className="text-slate-900 text-2xl font-semibold leading-tight tracking-[-0.015em] mb-4">
                앱 통계
              </h3>
                ...
                <div className="flex flex-col gap-2 rounded-xl p-6 border border-slate-200 bg-white shadow-sm">
                  ...
                  <p className="text-slate-900 text-3xl font-bold leading-tight">
                    {rows.length > 0
                      ? Math.round(
                          rows.reduce(
                            (acc, row) => acc + (row.appDesc?.length || 0),
                            0
                          ) / rows.length
                        )
                      : 0}
